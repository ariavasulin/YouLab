...
Use the following information to understand how to create your document:
    - create your file under `.handoffs/TICKET_YYYY-MM-DD_HH-MM-SS_description.md`, where:
        - TICKET is the ticket number (omit including the underscore if no ticket)
    ...
    - Gather metadata by running `git rev-parse HEAD` for commit hash and `git branch --show-current` for branch name
    - Examples:
        - With ticket: `ENG-2166_2025-01-08_13-55-22_create-context-compaction.md`
...
repository: [Repository name from current directory]
...
### 3. Confirm Creation
After writing the handoff document, confirm to the user that it has been created.
...
<template_response>
Handoff created! You can resume from this handoff in a new session by asking the agent to read and continue from:
...
```
.handoffs/ENG-2166_2025-01-08_13-44-55_create-context-compaction.md
```
</example_response>
...
- **avoid excessive code snippets**. While a brief snippet to describe some key change is important, avoid large code blocks or diffs; do not include one unless it's necessary (e.g. pertains to an error you're debugging). Prefer using `/path/to/file.ext:line` references that an agent can follow later when it's ready, e.g. `src/app/dashboard/page.tsx:12-24`
